...
export default function EditCarPublicPage() {
    ...
    const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
        ...
        // Backend xavfsizlik uchun secret_key talab qilsa, uni ham birga yuboramiz
        formData.append("secret_key", secretKey);

        // Yangi rasm tanlanmagan bo'lsa, eskisini saqlab qolish mantiqi
        ...
        try {
            ...
            if (res.ok) {
                ...
                router.push("/elon-berish"); // Oddiy foydalanuvchi qaytib shu sahifaga o'tadi
                router.refresh();
            } else {
                ...
                <div>
                    <div className="mb-8 md:mb-10 text-center">
                        <h1 className="text-3xl md:text-4xl font-[900] italic uppercase tracking-tighter text-zinc-900">
                            E'LONNI <span className="text-red-600">TAHRIRLASH</span>
                        </h1>
                    ...
                    <form onSubmit={handleSubmit} className="space-y-6">
                        <div className="bg-white p-6 md:p-12 rounded-[32px] md:rounded-[40px] shadow-[0_20px_50px_rgba(0,0,0,0.05)] border border-zinc-100">
                            ...
                            <div className="space-y-4 md:space-y-6">
                                ...
                                <div className="p-6 md:p-8 border-2 border-dashed border-zinc-200 rounded-[24px] bg-zinc-50 flex flex-col items-center gap-4 text-center overflow-hidden">
                                    ...
                                    <input
                                        ...
                                        type="file"
                                        accept="image/*"
                                        className="w-full max-w-xs text-[10px] md:text-xs file:mr-4 file:py-2.5 file:px-5 file:rounded-full file:border-0 file:text-[10px] file:font-black file:bg-zinc-900 file:text-white hover:file:bg-red-600 transition-all cursor-pointer mt-2"
                                    />
                                </div>
                            </div>
                            ...
                            <div className="flex flex-col sm:flex-row gap-3 sm:gap-4 mt-8 md:mt-10">
                                <button
                                    type="button"
                                    onClick={() => router.push("/elon-berish")}
                                ...
                                <button
                                    type="submit"
                                    disabled={loading}
                                    className="w-full sm:w-2/3 h-14 md:h-16 bg-red-600 hover:bg-red-700 text-white font-[900] uppercase tracking-[0.2em] rounded-[20px] md:rounded-[24px] shadow-xl transition-all active:scale-[0.98] disabled:opacity-50 text-[10px] md:text-xs"
                                >
