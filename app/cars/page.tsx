        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
          {data.length > 0 ? data.map((item: any) => (
            <div key={item.id} className="bg-white border border-zinc-200 rounded-2xl overflow-hidden shadow-sm hover:shadow-xl transition-all duration-300 group">
              ...
              <div className="p-4">
                <Link href={`/cars/${item.id}`}><h2 className="text-lg font-bold text-zinc-900 hover:text-red-600 transition-colors line-clamp-1">{item.name}</h2></Link>
                ...
                <Link href={`/cars/${item.id}`} className="block mt-4">
                  <button className="w-full py-2.5 bg-zinc-900 hover:bg-red-600 text-white text-xs font-bold uppercase rounded-xl transition-all active:scale-95">Batafsil ko'rish</button>
                </Link>
              </div>
            </div>
          )) : (
            <div className="col-span-full text-center py-20 text-zinc-400 font-bold italic uppercase">Hech narsa topilmadi</div>
          )}
        </div>
      </div>
    </main>
