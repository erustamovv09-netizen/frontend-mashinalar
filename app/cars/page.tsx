...
// Next.js 15+ uchun searchParams endi Promise sifatida keladi
export default async function Page({ 
  searchParams 
}: { 
  searchParams: Promise<{ search?: string }> 
}) {
  // 1. Birinchi navbatda searchParams ni await qilamiz (Terminaldagi xatoni yo'qotadi)
  const resolvedParams = await searchParams;
  const query = resolvedParams.search?.toLowerCase() || "";

  const data = await getData();

  // 2. Ma'lumotlarni filterlash
      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
          {filteredData.length > 0 ? (
            filteredData.map((item: any) => (
              <div key={item.id} className="bg-white border border-zinc-200 rounded-2xl overflow-hidden shadow-sm hover:shadow-xl transition-all duration-300 group">
                ...
                <div className="relative h-48 overflow-hidden">
                  ...
                  />
                  <div className="absolute top-3 left-3 bg-red-600 text-white text-[10px] font-bold px-2 py-1 rounded-md uppercase">Sotuvda</div>
                </div>
            ...
            <div className="col-span-full text-center py-20 bg-white rounded-[40px] border border-dashed border-zinc-300">
              <p className="text-zinc-400 font-black uppercase italic text-xl">Afsuski, bunday mashina topilmadi...</p>
              <Link href="/cars" className="text-red-600 font-bold mt-4 inline-block hover:underline">
                Barcha mashinalarga qaytish
              </Link>
            </div>
          )}
        </div>
