...
const slides = [
  ...
  },
  {
    ...
    link: "/admin", // E'lon berish (admin) qismiga
  ...
  },
  {
    ...
    link: "#footer", // Kontakt (pastga) tushadi
    btnText: "Bog'lanish"
  }
];
...
export default function Hero() {
  ...
  return (
    <div className="relative w-full h-[500px] md:h-[600px] bg-zinc-100 overflow-hidden">
      {slides.map((slide, index) => (
        ...
        >
          ...
          <div className="absolute inset-0">
            ...
            />
            {/* Sening saytingga mos oq overlay */}
          ...
          <div className="relative h-full container mx-auto px-6 flex flex-col justify-center items-start">
            <div className="max-w-2xl space-y-5">
              ...
              <p className="text-zinc-600 text-sm md:text-lg font-medium max-w-md">
                {slide.description}
              </p>

              <div className="pt-4">
                <Link href={slide.link}>
                  <button className="bg-zinc-900 hover:bg-red-600 text-white font-black uppercase px-8 py-4 rounded-2xl transition-all duration-300 active:scale-95 text-xs tracking-widest shadow-xl shadow-zinc-200">
                    {slide.btnText}
                  </button>
                </Link>
              </div>
        ...
        {slides.map((_, i) => (
          <button
            ...
            className={`h-1.5 transition-all duration-500 rounded-full ${
              current === i ? "w-8 bg-red-600" : "w-2 bg-zinc-300"
            }`}
          />
        ))}
      </div>
